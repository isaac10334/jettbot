...
export class UserTrackActor {
  ...
  start() {
    ...
    const supervise = () => {
      ...
      if (this.lastOpusAt && dt > this.opts.resubscribeAfterMs) {
        this.stallCount++
        if (this.opts.debug) {
          console.log(`[track] ${this.userId} stalled (${dt}ms) -> resubscribe`)
        }
        this.bumpBackoff()
  ...
  private subscribeFresh(reason: 'startup' | 'stall' | 'decode_timeout' | 'error') {
    if (this.stopped) return
    ...
    const next: Pipeline = { opusIn, decoder, stereoChunker, downmix, monoChunker }
    this.pipeline = next
    ...
    // Wire pipeline
    opusIn.pipe(decoder).pipe(stereoChunker).pipe(downmix).pipe(monoChunker)
    ...
    monoChunker.on('data', (monoFrame: Buffer) => {
      ...
      if (!sawFirstFrame) {
        ...
        if (this.opts.debug) {
          console.log(`[track] ${this.userId} resubscribe ok (gen=${gen}, reason=${reason})`)
        }
      }
    })
    ...
    const onErr = (label: string) => (err?: unknown) => {
      ...
      if (this.opts.debug) {
        console.log(`[track] ${this.userId} ${label} error -> resubscribe`, err ?? '')
      }
    ...
    setTimeout(() => {
      ...
      if (this.opts.debug) {
        console.log(`[track] ${this.userId} decode timeout (opus but no pcm) -> resubscribe`)
      }
  ...
  private bumpBackoff() {
    ...
    const next = this.backoffMs ? Math.min(2000, Math.floor(this.backoffMs * 1.8)) : 250
    this.backoffMs = next
