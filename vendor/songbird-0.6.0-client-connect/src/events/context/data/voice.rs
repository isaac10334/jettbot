...
impl VoiceTick {
    /// Creates a voice tick from decoded speaking and silent SSRC sets.
    #[must_use]
    pub fn new(speaking: HashMap<u32, VoiceData>, silent: HashSet<u32>) -> Self {
        Self { speaking, silent }
    }
}
...
impl VoiceData {
    /// Creates voice data from an optional RTP packet and optional decoded PCM samples.
    #[must_use]
    pub fn new(packet: Option<RtpData>, decoded_voice: Option<Vec<i16>>) -> Self {
        Self {
            packet,
            decoded_voice,
        }
