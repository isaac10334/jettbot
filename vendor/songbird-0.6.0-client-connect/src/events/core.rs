...
/// Songbird can observe when a user connects to the voice gateway ([`ClientConnect`]),
/// when a user *speaks for the first time* ([`SpeakingStateUpdate`]),
...
///
/// To obtain a user's SSRC, use [`ClientConnect`] or [`SpeakingStateUpdate`] events.
///
...
#[non_exhaustive]
pub enum CoreEvent {
    /// Voice gateway client connection event containing the user's audio SSRC.
    ///
    /// Fired on receipt of Discord's client connect event from another host.
    ClientConnect,
